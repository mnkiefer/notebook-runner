      ...
      it(`Running all cells in ${nb}`, async function () {
        ...
        for (let i=0; i<notebook.cellCount; i++) {
            ...
            switch (kind) {
                case 2:
                    ...
                    if (code) {
                        ...
                        md += `${codeString}\n`;
                        if (!success) {
                            comment += `- Notebook <a href="'<SUMMARY_URL>'"><i>${nb}</i></a>:\n\n  ${codeString}\n`;
                        }
                    }
                    if (output) {
            ...
            if (!success) {
                failed = true;
                dataDir = '../data/failed';
                await fsp.mkdir(path.join(__dirname, dataDir), { recursive: true }).catch((err) => console.log(err));
        ...
        await fsp.writeFile(srcmdPath, `---\n\n# Notebook _${path.basename(srcnbPath)}_:\n\n${md}\n\n`, "utf8");
